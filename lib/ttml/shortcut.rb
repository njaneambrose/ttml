#= Shorcut module
#This is the shortcut store for TTML it contains shortened tags that promote faster development of projects.
#It provides flexibility in writing of projects with very minimal tags to represent long sentences
#==Example
#This is just a sample of them visit https://github.com/njaneambrose/ttml/blob/master/docs/documentation.html#short to see more
# sp      => span
# doc5    => <!DOCTYPE html>
    ...
    class Shortcut
      class << self
        ...
        def get_short(val)#nodoc
			if val == "d" then val = "div"
			...
			elsif val == "sel" then val = "select"
			elsif val == "opt" then val = "option"
			elsif val == "sm" then val = "small"
			elsif val == "str" then val = "strong"			
			  ...
			  id=val.index('-')+1;sz = val.size;t=val.index('ic');val=val=" "*id+'<link rel="icon" href="'+val[t+3..sz]+'"/>'+"\n"
				...
				t=val.index('p');sz = val.size;
				st = val[t+1..sz];
				sd = st.split(/=/);sd.each{|t| t.strip!}
				id=val.index('-')+1;val=val=" "*id+'<meta property="'+sd[0]+'" content="'+sd[1]+'"/>'+"\n"	
			end  
			return val
		end
	  end
